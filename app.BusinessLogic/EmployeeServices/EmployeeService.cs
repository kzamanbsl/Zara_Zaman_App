    ...
    public class EmployeeService : IEmployeeService
    {
        ...
        public async Task<bool> AddRecord(EmployeeViewModel vm)
        {
            try
            {
                ...
                var existingEmployee = await _iEntityRepository.AllIQueryableAsync().FirstOrDefaultAsync(c => c.UserName.Trim() == vm.UserName.Trim() && c.IsActive);

                if (existingEmployee != null)
                {
                    // Username is already taken, return false
                    return false;
                }

                // Username is available, proceed to add the new record

                
                Employee emp = new Employee();
                // Populate emp with data from vm...
                emp.Name = vm.Name;
